...
namespace triluatsoft.tls.Authorization.Users
{
    ...
    public class UserAppService : tlsAppServiceBase, IUserAppService
    {
        ...
        public async Task<GetFullNameIdDto> GetFullNameUserId(GetFullNameByIdInput input)
        {
            var temp = new GetFullNameIdDto();
            var curId = AbpSession.GetUserId();
            if (input.Id == 0)
            {
                input.Id = curId;
            }
            var user = await UserManager.GetUserByIdAsync(input.Id);
            temp.FullName = user.Surname + " " + user.Name;
            temp.Id = input.Id;
            return temp;
        }
